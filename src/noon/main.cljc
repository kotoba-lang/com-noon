(ns noon.main
  "Kotodama WASM entrypoint for the Noon clean-room actor (L4) — Clojure port.
  ...
  facts (`noon.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "noon_pro"
    :fields [:sku :title :price :currency :inventory]              :required [:sku :title]
    :coerce {:price :float :inventory :int}                       :refs {}}
   {:entity "Variant"      :plural "variants"      :id-prefix "noon_var"
    :fields [:productId :sku :price :optionValues] :required [:sku :price]
    :coerce {:price :float}          :refs {:productId "Product"}}
   {:entity "Order"     :plural "orders"     :id-prefix "noon_ord"
    :fields [:customerId :number :total :currency :status]        :required [:number :total]
    :coerce {:total :float} :refs {:customerId "Customer"}}
   {:entity "LineItem" :plural "lineitems" :id-prefix "noon_lin"
    :fields [:orderId :productId :quantity :unitPrice]          :required [:quantity :unitPrice]
    :coerce {:quantity :int :unitPrice :float}       :refs {:orderId "Order" :productId "Product"}}
   {:entity "Customer"   :plural "customers"   :id-prefix "noon_cus"
    :fields [:email :firstName :lastName :phone]   :required [:email :firstName]
    :coerce {}        :refs {}}
   {:entity "Collection"    :plural "collections"    :id-prefix "noon_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"noon.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "noon-compat" :tier tier :entities entities} 200])
